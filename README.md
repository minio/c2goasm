...
Here is a (deceptively simple albeit working) C function doing an AVX2 intrinsics computation:
```
...
Running `c2goasm` will generate the following Go assembly (in `MultiplyAndAdd_amd64.s`)
```
...
Which should be accompanied by the following Go code (in `MultiplyAndAdd_amd64.go`)
```
//go:noescape
func _MultiplyAndAdd(vec1, vec2, vec3, result unsafe.Pointer)

func MultiplyAndAdd(bgra, gray gocv.View) {
